...
data_dir = os.path.join(hello_pytorch_dir, "data", "img_align_celeba_2k")
out_dir = os.path.join(hello_pytorch_dir, "results", "log_gan")
...
if __name__ == '__main__':
    ...
    for epoch in range(num_epochs):
        for i, data in enumerate(train_loader):
            ...
            real_label = torch.full((b_size,), real_idx, dtype=torch.float, device=device)

            noise = torch.randn(b_size, nz, 1, 1, device=device)
            fake_img = net_g(noise)
            fake_label = torch.full((b_size,), fake_idx, dtype=torch.float, device=device)
