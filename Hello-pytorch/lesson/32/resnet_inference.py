...
if __name__ == "__main__":
    data_dir = os.path.join(hello_pytorch_dir, "data", "hymenoptera_data")
    img_dir = os.path.join(data_dir, "val", "bees")
    model_path = os.path.join(hello_pytorch_dir, "data", "resnet_checkpoint_14_epoch.pkl")
    ...
    with torch.no_grad():
        for idx, img_name in enumerate(img_names):
            ...
            print(f"{idx + 1}/{num_img}: {img_name} {time_s}s")

    print(f"\ndevice:{device} total time:{time_total}s mean:{time_total/num_img}s")
